...
func TestWhatANodeActuallyReachesFromTheRealRegistry(t *testing.T) {
	...
	// slack.send-msg is the one per-TOOL exception (toolset.Tools), reviewed
	...
	// not the namespace.
	want := []string{"ask", "ping", "present_plan", "slack.send-msg", "version"}
...
//
// The set was reviewed BY NAME. `slack.send-msg` passed that review — the bot
...
func TestWhatANodeMayARGUEFromTheRealRegistry(t *testing.T) {
	...
	want := map[string][]string{
		...
		// as the app.
		"slack.send-msg": {"attachment_type", "body", "thread", "to"},
	}
...
func TestTheAdminsOwnTokenCannotBeSelectedFromANode(t *testing.T) {
	if !slices.Contains(toolset.DeniedArgs("slack.send-msg"), "as") {
		t.Fatal("`as` is not denied on slack.send-msg. `as: \"admin\"` selects the admin's personal xoxp- token " +
