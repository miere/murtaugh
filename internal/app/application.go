...
import (
	...
	slacksendmsg "github.com/miere/murtaugh/internal/tools/slack/sendmsg"
	slackupdatemsg "github.com/miere/murtaugh/internal/tools/slack/updatemsg"
...
type Application struct {
	...
	// askFlow backs the `ask` tool's multi-question card and is shared with the
	// gateway, which routes the answerer's clicks back into the blocked call.
	askFlow *askcard.Flow
...
func New(mode Mode, args []string, cfg config.Config, cfgStore config.Store, configPath, version string, logger *slog.Logger, recorder journal.Recorder, agents Agents) *Application {
	...
	// The ask card flow is shared the same way again: the `ask` tool blocks on it
	// while the gateway routes the answerer's clicks back in.
	askFlow := askcard.New(
		slacklib.NewLazyClient(cfg.OAuth.BotToken),
		askcard.NewRenderer(baseDirFor(cfg, configPath), assets.FS),
	)
	reg := buildRegistry(cfg, cfgStore, configPath, version, recorder, broker, askFlow, agents)
	return &Application{
		agents:            agents,
		mode:              mode,
		args:              args,
...
func buildRegistry(cfg config.Config, cfgStore config.Store, configPath, version string, recorder journal.Recorder, broker *interaction.Broker, askFlow *askcard.Flow, agents Agents) *tools.Registry {
	...
	journalOpener := func() (*journal.Store, error) {
		return journal.Open(cfg.Journal.EffectivePath(cfg.BaseDir, cfg.BaseName), cfg.Journal.RetentionByStream(),
