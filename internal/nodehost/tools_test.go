...
func TestTheGatewayRefusesAToolANodeMayNotReach(t *testing.T) {
	registry := tools.NewRegistry()
	registry.Register(ping.New())
	registry.Register(&namedTool{name: "slack.fetch_msgs"})
	...
	for _, forbidden := range []string{"slack.fetch_msgs", "node.token.mint", "cfg.agent.create"} {
	...
	err := callByName(t, rig, "slack.fetch_msgs")
	if err == nil {
...
// discards the text on purpose — so `slack.send_msg` has to cross or every job
...
func TestSendMsgCrossesButItsGatewayCredentialsAndFilePathsDoNot(t *testing.T) {
	...
	published := rig.proxy.Registry()
	tool, ok := published.Get("slack.send_msg")
	if !ok {
		t.Fatal("slack.send_msg did not cross, so a job on a node cannot report its own result")
	}
	if _, ok := published.Get("slack.fetch_msgs"); ok {
	...
	if _, err := rig.proxy.Call(context.Background(), "slack.send_msg", map[string]any{"channel": "C1", "text": "the backup finished"}); err != nil {
	...
	// attached".
	_, err := rig.proxy.Call(context.Background(), "slack.send_msg",
...
// recordingSendMsg stands in for the real slack.send_msg: the same name, the
...
func (t *recordingSendMsg) Name() string        { return "slack.send_msg" }
