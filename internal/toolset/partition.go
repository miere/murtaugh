...
var Families = []Family{
	...
	// ---- the gateway's alone ----------------------------------------------
	{Name: "slack", Reach: ReachGatewayOnly, Why: "holds the daemon's bot token AND the admin's personal user token, and send-msg's attachment/blocks arguments are unrooted GATEWAY filesystem paths it will read and upload"},
	// slack.send-msg is the one exception, and it is per TOOL rather than per
...
var Tools = []Tool{
	...
	// (fetch-msgs, fetch-reactions), editing somebody else's message
	// (update-msg), creating channels and writing canvases are not what a job
	// reporting its own result needs, and each is a wider capability than the
	// one being granted here.
	{
		Name: "slack.send-msg", Reach: ReachNode,
