...
func (f *Frontend) lookup(tokens []string) (string, bool) {
	if joined := strings.Join(tokens, "."); f.hasTool(joined) {
		return joined, true
	}
	want := looseName(strings.Join(tokens, " "))
	for _, t := range f.registry.All() {
		if looseName(t.Name()) == want {
			return t.Name(), true
		}
	}
	return "", false
...
func (f *Frontend) hasTool(name string) bool {
	_, ok := f.registry.Get(name)
	return ok
}
