...
func TestTheAggregatorReadsTheRegistryWhenASessionStartsNotWhenItIsBuilt(t *testing.T) {
	...
	aggr, err := newACPAggregator(srv, reg, resolvedFor(t, "", "ask", "ping"), nil, nil, nil)
	if err != nil {
	...
	if _, _, err := aggr.RegisterSession(agent.SessionMetadata{ChannelID: "C1", ThreadTS: "1.2"}); err != nil {
