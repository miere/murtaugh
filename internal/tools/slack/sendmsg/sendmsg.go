...
// tokens. botToken (oauth.bot_token in config.yaml) backs the default "as
...
func (t *Tool) Invoke(ctx context.Context, args map[string]any) (any, error) {
	...
	case "admin":
		if t.adminClient == nil {
			return nil, fmt.Errorf("Error: as=admin requires a Slack user token; set oauth.user_token in config.yaml")
		}
