...
func (c *SlackClient) DeleteMessage(ctx context.Context, p DeleteMessageParams) error {
	if _, _, err := c.api.DeleteMessageContext(ctx, p.ChannelID, p.TS); err != nil {
		return slackError("chat.delete", err)
	}
	return nil
}
