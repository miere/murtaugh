...
// notifyAdminAlert is the only way a lifecycle announcement reaches the admin's
// DM, so it has to honour the level rather than always posting a card: a notice
// through the admin DM must be the same one-line context message it is
// everywhere else.
func TestNotifyAdminAlertRoutesOnTheLevel(t *testing.T) {
	...
	newGW(msg, poster).notifyAdminAlert(context.Background(), configReloadedAlert())
	if len(poster.posts) != 0 {
		t.Fatalf("the notice was posted as a card; it must take the notice path")
	}
	if msg.postCalls != 1 {
		t.Fatalf("expected one notice post to the admin DM, got %d", msg.postCalls)
	}
	if !strings.Contains(msg.postText, configReloadedAlert().Title) {
		t.Errorf("notice text %q does not carry the alert's title", msg.postText)
	}

	// An info alert still gets the card it was always getting.
	msg, poster = &recordingMessaging{}, &fakeAlertAPI{}
	newGW(msg, poster).notifyAdminAlert(context.Background(), pongAlert())
