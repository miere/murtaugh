...
decision in front of the user (`ask` / `present_plan`, or the terminal approval
gate) and block until they respond. **Enabling** those tools is config: list
`ask` / `present_plan` in a profile's `tools:`, and set the terminal gate's
behaviour with the `approval:` block (`allowlist` / `prompt` / `off`) — both in
`reference/agents-yaml.md`. **How the agent *uses* them** (and the consent
posture behind them) lives in the `murtaugh-slack` skill (`reference/asking.md`).

The gate is only active in **live chat** — scheduled jobs and delegated agents
are never gated, and the interactive tools error rather than block outside a
Slack conversation.
